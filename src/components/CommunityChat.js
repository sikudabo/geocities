...
function CommunityChat(props) {
    ...
    const [socket] = useSocket('http://10.162.93.179:3001/'); //useSocket hook to maintain a connection.
    ...
    function deleteMsg(uniqueMessageId) {
        ...
        return axios({
            method: 'POST',
            url: 'http://10.162.93.179:3001/api/delete/chat/msg',
            data: data,
            headers: {
                            ...
                            {community.chatRoom.messages.map((msg, index) => (
                                <div>
                                    ...
                                    >
                                        <ListItemAvatar>
                                            <Avatar 
                                                src={`http://10.162.93.179:3001/api/get/avatar/by/id/${msg.uniqueUserId}`}
                                                title={`${msg.username}`}
                                                alt={`${msg.username}`} 
